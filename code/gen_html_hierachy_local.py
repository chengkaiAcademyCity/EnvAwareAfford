...
def func(output_folder, num_per_page, input_main_folder, input_folder_list_comma, input_title_list_comma,
         first_hierachy, fout, prompt):
    ...
    if num_hierachy > 1:
        ...
        for i in range(2, num_hierachy):
            input_folder_list_others += ':' + input_folder_list_hierachy[i]
            input_title_list_others += ':' + input_title_list_hierachy[i]
    ...
    if os.path.isdir(os.path.join(input_main_folder, 'ids')):
        filelist = [int(f.split('.')[0]) for f in os.listdir(os.path.join(input_main_folder, 'ids')) if
                    f.endswith('.id')]
        filelist.sort()
        ...
        filelist = [f.split('.')[0] for f in os.listdir(os.path.join(input_main_folder, input_folder_list[0])) if
    ...
    for f in file_name_list:
        ...
        if first_hierachy and k % num_per_page == 0:
            ...
            fout.write(
                '<h3><a href="{}">First</a>&nbsp;&nbsp;&nbsp;<a href="{}"{}>Prev</a>&nbsp;&nbsp;&nbsp;<a href="{}"{}>Next</a>&nbsp;&nbsp;&nbsp;<a href="{}">Last</a></h3>'.format(
                    str(0) + '.html', str(prev_id) + '.html', prev_click, str(next_id) + '.html', next_click,
        ...
        for input_folder in input_folder_list:
            if os.path.exists(os.path.join(input_main_folder, input_folder, str(f) + '.jpg')):
                ...
                print('Warning: there is no .jpg/.png/.txt file whose name starts with %s!' % os.path.join(
                    input_main_folder, input_folder, str(f)))
                data[counter] = ('none', '')
        ...
        if num_hierachy > 1:
            ...
            func(output_folder, 1000, os.path.join(input_main_folder, 'child', str(f)), input_folder_list_others,
                 input_title_list_others, False, fout, prompt + '\t')
            fout.write('</div>')
            fout.write(
                '<button style="margin: 1em 1em 1em 1em; font-size: 15px; border-radius:7px;">Click Here to See/Hide More Details</button>')
        fout.write('</div>')
