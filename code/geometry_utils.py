"""
    This code uses https://github.com/sidch/Thea
"""
...
def export_pts(out, v):
    with open(out, 'w') as fout:
        for i in range(v.shape[0]):
            fout.write('%f %f %f\n' % (v[i, 0], v[i, 1], v[i, 2]))
    # tmp = out.replace('.obj', '')
    # cmd = 'xvfb-run -a ~/thea/TheaDepsUnix/Source/TheaPrefix/bin/Thea/RenderShape %s %s.png 448 448' % (tmp, tmp)
    # print(cmd)
    # call(cmd, shell=True)
...
def sample_points_from_obj(label_fn, obj_fn, pts_fn, num_points, verbose=False):
    cmd = 'xvfb-run -a /usr/local/bin/Thea/MeshSample -n%d -s3 -l %s %s %s> /dev/null' % (num_points, label_fn, obj_fn, pts_fn)
    # cmd = 'xvfb-run -a ~/thea/TheaDepsUnix/Source/TheaPrefix/bin/Thea/RenderShape %s.pts -f %s.feats %s.png 448 448 -v 1,0,0,-5,0,0,0,0,1 >> /dev/null' % (
    # out, out, out)
...
def render_obj(out, v, f, delete_img=False, flat_shading=True, delete_tmp_obj=True):
