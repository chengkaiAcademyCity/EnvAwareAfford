...
class Robot(object):
    def __init__(self, env, urdf, init_scale, material, open_gripper=False):
        ...
        self.end_effector_index, self.end_effector = \
            [(i, l) for i, l in enumerate(self.robot.get_links()) if l.name == 'gripper_base_link'][0]
        self.hand_actor_id = self.end_effector.get_id()
        self.gripper_joints = [joint for joint in self.robot.get_joints() if 
                joint.get_name().startswith("left_finger_tip_joint")]
        ...
        self.arm_joints = [joint for joint in self.robot.get_joints() if
                joint.get_dof() > 0 and not joint.get_name().startswith("gripper_base_link")]
    ...
    def setup_planner(self):
        ...
        self.planner = mplib.Planner(
            urdf="../assets/robot/xarm7/xarm7.urdf",
            srdf="../assets/robot/xarm7/xarm7.srdf",
            user_link_names=link_names,
            user_joint_names=joint_names,
            move_group="gripper_base_link",
