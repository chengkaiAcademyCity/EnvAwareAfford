...
from data_trip import SAPIENVisionDatasetTrip
...
def train(conf, train_shape_list, train_data_list, val_data_list, all_train_data_list, contrastive_dir_p, contrastive_dir_n):
    # create training and validation datasets and data loaders
    data_features = ['scene_pc_cam', 'cam2cambase', 'gt_applicable_pc', 'gt_object_pc', \
                     'rgb', 'rgb_point', 'gt_applicable_img', 'gt_possible_img', 'gt_object_img', \
                     'gt_nor', 'robot_p', \
                     'result', 'cur_dir', 'shape_id', 'occlusion_shape_id']
    ...
    if (not conf.resume) and (conf.pretrained_ckpt is not None):
        utils.printout(conf.flog, f'Loading pretrained Critic ckpt {conf.pretrained_ckpt}')
        data_to_restore = torch.load(conf.pretrained_ckpt)
    ...
    # load dataset
    train_dataset = SAPIENVisionDatasetTrip(conf.category_types, data_features, \
                                        env_name=conf.env_name, buffer_max_num=conf.buffer_max_num,
                                        img_size=conf.img_size, \
                                        no_true_false_equal=conf.no_true_false_equal, \
                                        no_aug_neg_data=conf.no_aug_neg_data, contrastive_dir_p=conf.contrastive_positive_data_dir, contrastive_dir_n=conf.contrastive_negative_data_dir)
    ...
    for epoch in range(start_epoch, conf.epochs):
        ...
        if epoch > start_epoch:
            ...
            for item in all_train_data_list:
                ...
                if valid_id_l <= int(item.split('_')[-3]) < valid_id_r:
        ...
        if conf.sample_succ:
            if conf.resume and epoch == start_epoch:
                sample_succ_list = torch.load(
                    os.path.join(conf.exp_dir, 'ckpts', '%d-sample_succ_list.pth' % start_epoch))
            else:
                torch.save(sample_succ_list, os.path.join(conf.exp_dir, 'ckpts', '%d-sample_succ_list.pth' % epoch), _use_new_zipfile_serialization=False)
        ...
        if conf.resume and epoch == start_epoch:
            ...
            preloaded_train_dataset_path = os.path.join(conf.log_dir,
                                                        'exp-%s-model_3d_critic-None-train_3d_critic' % conf.env_name,
                                                        'ckpts', '0-train_dataset.pth')
        ...
        train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=conf.batch_size, shuffle=True,
                                                       pin_memory=True, \
                                                       num_workers=4, drop_last=True, collate_fn=utils.collate_feats_pass,
                                                       worker_init_fn=utils.worker_init_fn)
        ...
        for train_batch_ind, batch in train_batches:
            ...
            if train_batch_ind == 0:
                with torch.no_grad():
                    ...
                    torch.save(network.state_dict(), os.path.join(conf.exp_dir, 'ckpts', '%d-network.pth' % epoch), _use_new_zipfile_serialization=False)
                    torch.save(network_opt.state_dict(),
                               os.path.join(conf.exp_dir, 'ckpts', '%d-optimizer.pth' % epoch), _use_new_zipfile_serialization=False)
                    torch.save(network_lr_scheduler.state_dict(),
                               os.path.join(conf.exp_dir, 'ckpts', '%d-lr_scheduler.pth' % epoch), _use_new_zipfile_serialization=False)
                    torch.save(train_dataset, os.path.join(conf.exp_dir, 'ckpts', '%d-train_dataset.pth' % epoch), _use_new_zipfile_serialization=False)
                    utils.printout(conf.flog, 'DONE')
            ...
            total_loss, end_points = forward(batch=batch, data_features=data_features, network=network, conf=conf,
                                             is_val=False, \
                                             step=train_step, epoch=epoch, batch_ind=train_batch_ind,
...
def forward(batch, data_features, network, conf, \
    ...
    if not is_val:
        ...
        bs2 = len(batch[0][0])
        # print(bs0, bs1, bs2)
        new_batch = [[] for i in range(bs2)]
        for i in range(bs0):
            for j in range(bs1):
                tmp = batch[i][j]
                for k in range(bs2):
                    # print(len(tmp))
                    new_batch[k].append(tmp[k])
        # ipdb.set_trace()
        batch = new_batch
    ...
    gt_object_pc = torch.cat(batch[data_features.index('gt_object_pc')], dim=0).to(conf.device)  # B x 3N
    batch_size = input_scene_pcs_cam.shape[0]
    # print(input_scene_pcs_cam.shape)
    # fps to 10K-points
    ...
    input_scene_pcs_cam = input_scene_pcs_cam[input_pcid1, input_pcid2, :].reshape(batch_size, conf.num_point_per_shape,
                                                                                   -1)  # B x N x 3
    gt_applicable_pc = gt_applicable_pc[input_pcid1, input_pcid2].reshape(batch_size, -1)  # B x N
    gt_object_pc = gt_object_pc[input_pcid1, input_pcid2].reshape(batch_size, -1)  # B x N
    ...
    pc_centers = (input_scene_pcs_cambase.max(dim=1, keepdim=True)[0] +
                  input_scene_pcs_cambase.min(dim=1, keepdim=True)[0]) / 2
    input_scene_pcs_cambase -= pc_centers
    robot_p = torch.Tensor(batch[data_features.index('robot_p')]).to(conf.device)  # B x 3
    robot_p -= pc_centers.squeeze(1)

    pred_result_logits, subsampled_scene_feats, _ = network(input_scene_pcs_cambase,
                                                                     robot_p, gt_object_pc)  # B x 2, B x F x N
    # prepare gt
    ...
    trip_loss = torch.Tensor([0]).to(conf.device)
    if not is_val:
        batch_size = subsampled_scene_feats.shape[0]
        data_size = batch_size // 3
        anchor, pos, neg = [], [], []
        for idx_data in range(data_size):
            anchor.append(subsampled_scene_feats[idx_data * 3].view(1, -1))
            pos.append(subsampled_scene_feats[idx_data * 3 + 1].view(1, -1))
            neg.append(subsampled_scene_feats[idx_data * 3 + 2].view(1, -1))
        anchor = torch.cat(anchor, dim=0)
        pos = torch.cat(pos, dim=0)
        neg = torch.cat(neg, dim=0)

        triplet_loss = torch.nn.TripletMarginLoss(margin=2.0, p=2, reduction='mean')
        trip_loss = triplet_loss(anchor, pos, neg)

    total_loss = result_loss + trip_loss
    # display information
    data_split = 'train'
    if is_val:
        data_split = 'val'
    ...
    with torch.no_grad():
        ...
        if log_console:
            utils.printout(conf.flog, \
                           ...
                           f'''{trip_loss.item():>10.5f}'''
                           f'''{total_loss.item():>10.5f}''')
            conf.flog.flush()

        # log to tensorboard
        if log_tb and tb_writer is not None:
            tb_writer.add_scalar('total_loss', result_loss.item(), step)
            tb_writer.add_scalar('real_total_loss', total_loss.item(), step)
            tb_writer.add_scalar('trip_loss', trip_loss.item(), step)
            tb_writer.add_scalar('lr', lr, step)

        # gen visu
        if is_val and (not conf.no_visu) and epoch % conf.num_epoch_every_visu == 0:
            pred_result_logits, subsampled_scene_feats, _ = network(
                input_scene_pcs_cambase,
                robot_p, gt_object_pc)
            ...
            input_scene_pc_cam_dir = os.path.join(out_dir, 'scene_pc_cam')
            input_scene_pc_cambase_dir = os.path.join(out_dir, 'scene_pc_cambase')
            input_acting_pc_cam_dir = os.path.join(out_dir, 'robot_pc_cam')
            input_acting_pc_cambase_dir = os.path.join(out_dir, 'robot_pc_cambase')
            ...
            viz_weight_dir = os.path.join(out_dir, 'viz_weight')
            ...
            if batch_ind < conf.num_batch_every_visu:
                ...
                for i in range(batch_size):
                    ...
                    utils.render_pts(os.path.join(BASE_DIR, input_scene_pc_cam_dir, fn.split('.')[0]),
                                     input_scene_pcs_cam[i].cpu().numpy(), highlight_id=0, campos=0)
                    utils.render_pts(os.path.join(BASE_DIR, input_scene_pc_cambase_dir, fn.split('.')[0]),
                                     input_scene_pcs_cambase[i].cpu().numpy(), highlight_id=0)
                    utils.render_pts_label_png(os.path.join(BASE_DIR, pred_all_result_dir, fn.split('.')[0]),
                                               input_scene_pcs_cam[i].cpu().numpy(), pred_all_result[i].cpu().numpy(),
                                               campos=0)
                    ...
                    with open(os.path.join(info_dir, fn.replace('.png', '.txt')), 'w') as fout:
                        ...
                        fout.write('pred_true_score: %f\n' % pred_result_true_score[i].cpu().numpy())
            ...
            if batch_ind == conf.num_batch_every_visu - 1:
                ...
                sublist = 'scene_pc_cambase,robot_pc_cambase,scene_pc_cam,robot_pc_cam,pred_all_result,viz_rgb,gt_applicable,gt_possible,viz_rgb_point,viz_weight,info'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--category_types', type=str,
                        help='list all categories [Default: None, meaning all categories]', default=None)
    ...
    parser.add_argument('--contrastive_positive_data_dir', type=str, help='data directory', default=None)
    parser.add_argument('--contrastive_negative_data_dir', type=str, help='data directory', default=None)
    parser.add_argument('--train_data_fn', type=str, help='data directory', default='data_tuple_list.txt')
    ...
    parser.add_argument('--pretrained_ckpt', type=str, help='pretrained ckpt', default=None)
    ...
    parser.add_argument('--closet_warm_up_epoch', type=int, default=0)
    ...
    if conf.data_dir_prefix is None:
        conf.data_dir_prefix = '../data/gt_%s' % conf.env_name
    if conf.offline_data_dir is None:
        conf.offline_data_dir = conf.data_dir_prefix + '_train_data-pushing'
    if conf.val_data_dir is None:
        conf.val_data_dir = conf.data_dir_prefix + '_test_data-pushing'
    ...
    # save config
    if not conf.resume:
        torch.save(conf, os.path.join(conf.exp_dir, 'conf.pth'), _use_new_zipfile_serialization=False)
    ...
    with open(os.path.join(conf.offline_data_dir, conf.train_data_fn), 'r') as fin:
    ...
    if conf.resume:
        ...
        train_data_list = []
        for item in all_train_data_list:
            if int(item.split('_')[-3]) < conf.num_interaction_data_offline:
                train_data_list.append(item)
    ...
    train_shape_list = None

    ### start training
    train(conf, train_shape_list, train_data_list, val_data_list, all_train_data_list, conf.contrastive_positive_data_dir, conf.contrastive_negative_data_dir)
