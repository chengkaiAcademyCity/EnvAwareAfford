...
parser.add_argument('--data_fn', type=str, help='data directory', default='data_tuple_list.txt')
...
parser.add_argument('--buffer_max_num', type=int, default=100000)
parser.add_argument('--no_visu', action='store_true', default=False, help='no visu? [default: False]')
parser.add_argument('--whole_visu', action='store_true', default=False, help='whole pc visu? [default: False]')
...
parser.add_argument('--only_true_data', action='store_true', default=False, help='only show true cases [default: False]')
...
if not eval_conf.no_visu:
    ...
    viz_weight_dir = os.path.join(visu_dir, 'viz_weight')
...
data_features = ['scene_pc_cam', 'cam2cambase', 'gt_applicable_pc', 'gt_object_pc', \
            'rgb', 'rgb_point', 'gt_applicable_img', 'gt_possible_img', \
            'gt_nor', 'robot_p', 'rgb_start', 'robot_pc_cam', \
            'result', 'cur_dir', 'shape_id', 'occlusion_shape_id']

with open(os.path.join(eval_conf.data_dir, eval_conf.data_fn), 'r') as fin:
...
dataset = SAPIENVisionDataset(category_types, data_features, \
        ...
        no_aug_neg_data=True, buffer_max_num=eval_conf.buffer_max_num, only_true_data=eval_conf.only_true_data)
...
with torch.no_grad():
    ...
    for batch_id, batch in enumerate(dataloader, 0):
        ...
        # input_scene_pcs_cam = torch.cat(batch[data_features.index('object_pc_cambase')], dim=0).to(eval_conf.device)  # B x 3N x 3

        gt_applicable_pc = torch.cat(batch[data_features.index('gt_applicable_pc')], dim=0).to(eval_conf.device) # B x 3N
        gt_object_pc = torch.cat(batch[data_features.index('gt_object_pc')], dim=0).to(eval_conf.device)  # B x 3N
        ...
        input_acting_pcs_cam = torch.cat(batch[data_features.index('robot_pc_cam')], dim=0).to(eval_conf.device)  # B x N' x 3
        input_acting_pcs_cambase = torch.matmul(input_acting_pcs_cam, cam2cambase_rotmats.permute(0, 2, 1))

        robot_p = torch.Tensor(batch[data_features.index('robot_p')]).to(eval_conf.device)  # B x 3
        ...
        try:
            pred_result_logits, condition_subsampled_feats, visu_weights, subsampled_scene_seeds, end_points, subsampled_object_pc = network(input_scene_pcs_cambase,
                                                                                            robot_p, gt_object_pc)  # B, B x P x 3, B x P x 1
            visu_seeds = torch.matmul(subsampled_scene_seeds, cam2cambase_rotmats.permute(0, 2, 1).inverse())
        except:
            try:
                pred_result_logits, _, end_points = network(input_scene_pcs_cambase, robot_p, gt_object_pc) # B
            except:
                pred_result_logits, end_points = network(input_scene_pcs_cambase, robot_p)  # B

        # pred_result_logits, end_points = network(input_scene_pcs_cambase, robot_p)  # B x 2, B x F x N
        if not eval_conf.no_visu and eval_conf.whole_visu:
            # inference all results
            torch.cuda.empty_cache()
            pred_all_result = network.inference_whole_pc(input_scene_pcs_cambase, robot_p, gt_object_pc)
            torch.cuda.empty_cache()
        ...
        # prepare viz
        viz_rgb = torch.cat(batch[data_features.index('rgb')], dim=0)     # B x 3 x H x W
        gt_applicable_img = torch.cat(batch[data_features.index('gt_applicable_img')], dim=0) # B x H x W
        gt_possible_img = torch.cat(batch[data_features.index('gt_possible_img')], dim=0)     # B x H x W
        viz_rgb_point = torch.cat(batch[data_features.index('rgb_point')], dim=0)     # B x 3 x H x W
        viz_rgb_start = torch.cat(batch[data_features.index('rgb_start')], dim=0)     # B x 3 x H x W
        dir_seq = batch[data_features.index('cur_dir')]
        ...
        if (not eval_conf.no_visu) and batch_id < eval_conf.visu_batch:
            ...
            for i in range(viz_batch_size):
                if (not eval_conf.show_failure) or ((pred_result_true_score[i].item() > eval_conf.true_thres) != (gt_result[i].item() > 0.5)):
                    fn = dir_seq[i].split('/')[-1] + 'data-%03d.png' % (batch_id * viz_batch_size + i)
                    try:
                        weight_min = torch.min(visu_weights[i])
                        weight_max = torch.max(visu_weights[i])
                        visu_weights[i] = (visu_weights[i] - weight_min) / (weight_max - weight_min)
                        utils.render_pts_label_png_with_p_size(os.path.join(viz_weight_dir, 'weight-' + fn.split('.')[0]),
                                                   visu_seeds[i].cpu().numpy(), visu_weights[i].cpu().numpy(), 10)
                    except NameError:
                        pass
                    pred_min = torch.min(pred_all_result[i])
                    pred_max = torch.max(pred_all_result[i])
                    pred_all_result[i] = (pred_all_result[i] - pred_min) / (pred_max - pred_min)
            ...
            if batch_id == eval_conf.visu_batch - 1:
                ...
                if eval_conf.actobj_random_size_rotation:
                    sublist = 'scene_pc_cambase,robot_pc_cambase,scene_pc_cam,robot_pc_cam,pred_all_result,viz_rgb,gt_applicable,gt_possible,viz_rgb_point,viz_rgb_start,viz_weight,info'
                else:
                    sublist = 'scene_pc_cambase,robot_pc_cambase,scene_pc_cam,robot_pc_cam,pred_all_result,viz_rgb,gt_applicable,gt_possible,viz_rgb_point,,viz_rgb_start,viz_weight,info'
...
if not eval_conf.actobj_random_size_rotation:
    with open(os.path.join(result_dir, 'results.txt'), 'w') as fout:
        ...
        utils.printout(fout, 'precision: %f%%' % (precision * 100))
        utils.printout(fout, 'recall: %f%%' % (recall * 100))
        utils.printout(fout, 'F1 score: %f%%' % (F1score * 100))
